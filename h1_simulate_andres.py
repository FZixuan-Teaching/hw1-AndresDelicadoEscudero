# -*- coding: utf-8 -*-
"""h1_simulate_Andres.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KiJOWbGOJ294dPI_JNFXCpTyaKtnc_qR
"""
...
def greedy_algorithm(P: dict, D: dict, patient_status: dict, donor_status: dict, compatible_blood_type: dict = compatible_blood_type):
  ...
  for p in patients:
        for d in donors:
            if D[d] in compatible_blood_type[P[p]] and not donor_status[d]:
                matches.append((p, d))
                patient_status[p] = True
                donor_status[d] = True
                break  # Move to the next patient after finding a match
...
def mip(P: dict, D: dict, patient_status: dict, donor_status: dict, compatible_blood_type: dict):
    ...
    compatible_blood_type : dict
        For every patient blood type, this gives a list of donor blood types compatible with that patient
    """
    patients = [p for p in P.keys() if not patient_status[p]]
    donors = [d for d in D.keys() if not donor_status[d]]

    model = Model('kidneymatching')

    # Decision variables x[p, d] is 1 if patient p is matched with donor d, 0 otherwise
    x = {}
    for p in patients:
        for d in donors:
            if D[d] in compatible_blood_type[P[p]]:  # Check compatibility
                x[p, d] = model.addVar(vtype=GRB.BINARY, name=f"x_{p}_{d}")

    model.update()

    # Each patient can be matched to at most one donor
    for p in patients:
    ...
    model.setObjective(sum(x[p, d] for (p, d) in x), GRB.MAXIMIZE)
    ...
    matches = [(p, d) for (p, d) in x if x[p, d].x > 0.5]

    # Update patient and donor statuses
    for p, d in matches:
        patient_status[p] = True
        donor_status[d] = True
...
def simulate(
  ...
  # dictionary to accumulate the proportion of matched patients per period for each blood type
  prop_matched_by_type = {bt: 0 for bt in compatible_blood_type.keys()}
  ...
  for it in range(num_periods):
    ...
    for key in compatible_blood_type.keys():
      ...
      if curr_num_avail > 0:
            curr_avg_prop_matched = num_matched_by_type[key] / num_avail_patients_by_type[key]
      else:
            curr_avg_prop_matched = 1.0
      # Accumulate the proportion for this period.
      prop_matched_by_type[key] += curr_avg_prop_matched

      # Update the number of available patients for next period:
      # Subtract the ones that got matched in this period.
    ...
    for i in patients.keys():
      if patient_status[i]==False:
        TIS[i] += 1
  #Avg time in sistem by an average patient
  overall_avg_wait = sum(TIS[i] for i in TIS) / len(TIS)
  # v)
  avg_prop_matched = {bt: prop_matched_by_type[bt] / num_periods for bt in compatible_blood_type.keys()}
  # Report statistics
  avg_matches_bt={bt: round(num_matched_by_type[bt] / num_periods,2) for bt in num_matched_by_type}
  ...
  print('1.1(b)iii: Average number of patients (per blood type) matched per period:', avg_matches_bt)
  print('1.1(b)iv: Average time in system:', overall_avg_wait)
  ...
  print('1.1(b)v: Average proportion of patients matched per period by type:', avg_prop_matched)
